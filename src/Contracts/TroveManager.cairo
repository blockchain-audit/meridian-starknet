...
//     }
    //variables
    let borrowerOperationsAddress:ContractAddress;

    // IStabilityPool public override stabilityPool;

    let gasPoolAddress :felt252;

    let collSurplusPool:ICollSurplusPool;
    ...
    // ILQTYStaking public override lqtyStaking;
     ...
     pub sortedTroves :ISortedTroves;

    const SECONDS_IN_ONE_MINUTE :felt252= 60;
    const MINUTE_DECAY_FACTOR :felt252 = 999037758833783000;
    const REDEMPTION_FEE_FLOOR:felt252 = DECIMAL_PRECISION / 1000 * 5; // 0.5%
    const MAX_BORROWING_FEE : felt252= DECIMAL_PRECISION / 100 * 5; // 5%
    const BOOTSTRAP_PERIOD:felt252 = 14 days;
...
// Create the dictionary
    ...
    let  totalStakes:felt252;

//     // Snapshot of the value of totalStakes, taken immediately after the latest liquidation
    let totalStakesSnapshot:felt252;

//     // Snapshot of the total collateral across the ActivePool and DefaultPool, immediately after the latest liquidation.
    let totalCollateralSnapshot:felt252;
    const BETA = 2;
    let baseRate:felt252;
    // The timestamp of the latest fee operation (redemption or new LUSD issuance)
    let lastFeeOperationTime:felt252;
...
//     */
//     function _getCappedOffsetVals(uint256 _entireTroveDebt, uint256 _entireTroveColl, uint256 _price)
//         internal
//         pure
...
//             if (vars.ICR < MCR) {
//                 singleLiquidation =
//                     _liquidateNormalMode(_activePool, _defaultPool, vars.user, vars.remainingLUSDInStabPool);
...
//                     _activePool, _defaultPool, vars.user, vars.ICR, vars.remainingLUSDInStabPool, TCR, _price
//                 );
...
//             oldTotals.totalCollGasCompensation.add(singleLiquidation.collGasCompensation);
//         newTotals.totalLUSDGasCompensation =
//             oldTotals.totalLUSDGasCompensation.add(singleLiquidation.LUSDGasCompensation);
...
//     function _sendGasCompensation(IActivePool _activePool, address _liquidator, uint256 _LUSD, uint256 _ETH) internal {
//         if (_LUSD > 0) {
...
//             emit TroveUpdated(
//                 _borrower, newDebt, newColl, Troves[_borrower].stake, TroveManagerOperation.redeemCollateral
//             );
//         }
...
//     */
//     function _redeemCloseTrove(ContractsCache memory _contractsCache, address _borrower, uint256 _LUSD, uint256 _ETH)
//         internal
//     {
...
//             activePool, defaultPool, lusdToken, lqtyStaking, sortedTroves, collSurplusPool, gasPoolAddress
//         );
...
//     function _redistributeDebtAndColl(IActivePool _activePool, IDefaultPool _defaultPool, uint256 _debt, uint256 _coll)
//         internal
//     {
//         if (_debt == 0) return;
...
//         uint256 LUSDDebtNumerator = _debt.mul(DECIMAL_PRECISION).add(lastLUSDDebtError_Redistribution);
...
//     */
//     function _updateBaseRateFromRedemption(uint256 _ETHDrawn, uint256 _price, uint256 _totalLUSDSupply)
//         internal
//         returns (uint256)
//     {
...
//     function _calcBorrowingFee(uint256 _borrowingRate, uint256 _LUSDDebt) internal pure returns (uint256) {
...
//     function _requireLUSDBalanceCoversRedemption(ILUSDToken _lusdToken, address _redeemer, uint256 _amount)
//         internal
//         view
//     {
//         require(
