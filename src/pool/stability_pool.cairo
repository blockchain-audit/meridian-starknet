...
fn _requireFrontEndNotRegistered(_address: ContractAddress) {
    assert(
        !frontEnds[_address].registered, "StabilityPool: must not already be a registered front end"
    );
}
#[view]
...
fn _requireValidKickbackRate(_kickbackRate: u256) {
    assert(
        _kickbackRate <= DECIMAL_PRECISION, "StabilityPool: Kickback rate must be in range [0,1]"
    );
}
